$(document).ready(function () {
  ...
  $('#basic_message_close').on('click', function () {
    // Aquí va tu lógica
    $('#basic_message').hide();
  });
...
//dialog
function dialog_basic(title, text) {
  $('#basic_message_title').text(title);
  $('#basic_message_text').text(text);
  $('#basic_message').show();
}
  ...
  $.ajax({
    ...
    success: function (data) {
      if (data.status == "OK") {
        ...
        $('#loading').hide();
        //$('#service_names').show();
        dialog_basic("Error",data.status);
      }
    },
    error: function () {
      dialog_basic("Error","Error en ajax/" + name + ".php");
      $('#loading').show();
    }
  });
}
...
function enviar_mensaje(name, email, company, projectDescription) {
  ...
  //alert(name + " "+ email + " "+company + " "+ projectDescription);
  $.ajax({
    ...
    success: function (response) {
      if (response.status === 'OK') {
        ...
        dialog_basic("Gracias por tu propuesta!","Uno de nuestros agentes se comunicara contigo rapidamente");
      } else {
         dialog_basic("Gracias por tu propuesta!","un error inesperado");
        //dialog_basic('send_message failed: ' + response.message);
      }
      $('#loading').hide();
      //clear_dialog()
    },
    error: function () {
      dialog_basic("Error","Error en send_message.php");
      $('#loading').hide();
     // clear_dialog()
    }
  });
