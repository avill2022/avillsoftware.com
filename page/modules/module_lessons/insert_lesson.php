<?php
	...
	$query = "select * from lessons where email='$email' or lessonname='$name'";
	...
	try{
		...
		if($num > 0){
			...
			$query = "INSERT INTO lessons (lessonname,email,password,type)VALUES('$name','$email','$password',$type);";
			$response['status'] = $query;
			$res = mysqli_query($dbh,$query);
			if ($res){
				$response['status']='OK';
