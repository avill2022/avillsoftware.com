<?php
									...
									<div>
										...
										<div
											...
											<div data-side="bottom" data-align="center" role="menu" aria-orientation="vertical" data-state="open"
												...
												<div 
													id="edit_test'.$id.'"
													onclick="javascript:modal_edit_test('. $id .','.'\'' . addslashes($testname) .'\','.'\'' . addslashes($email) .'\','. $type .',\'' . addslashes($created_at) .'\')"
													role="menuitem"
												...
												<div id="delete_test'.$id.'"
													onclick="javascript:delete_test('.$id.')"
													role="menuitem"
	...
	try{
		$result=mysqli_query($dbh, "select * from tests;");
		...
		if ($result){
			if ($total>0){
				...
				while ($ren=mysqli_fetch_array($result)){
					$content_table.= get_item($ren['id'],$ren['testname'],$ren['jsonquestion'],$ren['created_at'],$ren['type']);
				}
			}
			else{
				$content_table='</tr>';
			...
			$table='<div style="opacity: 1; transform: none;">
				<div class="flex justify-between items-center mb-8">
					...
					<button
						id="modal_test"
						onClick="javascript:modal_insert_test()"
