<?php
	...
	function get_item($id,$username,$email,$created_at,$type):string{
		return '<tr class="border-b border-white/10 hover:bg-white/5 transition-colors">
                            ...
                            <td class="p-4 text-right">
							    <button
									onclick="javascript:toggle_menu_user(menu_user'.$id.')"
									...
									<div>
										...
										<div
											...
											<div data-side="bottom" data-align="center" role="menu" aria-orientation="vertical" data-state="open"
												...
												<div 
													id="edit_user'.$id.'"
													onclick="javascript:modal_edit_user('. $id .','.'\'' . addslashes($username) .'\','.'\'' . addslashes($email) .'\','. $type .',\'' . addslashes($created_at) .'\')"
													role="menuitem"
												...
												<div id="delete_user'.$id.'"
													onclick="javascript:delete_user('.$id.')"
													role="menuitem"
		...
		if ($result){
			if ($total>0){
				...
				while ($ren=mysqli_fetch_array($result)){
					$content_table.= get_item($ren['id'],$ren['username'],$ren['email'],$ren['created_at'],$ren['type']);
				}
			}
			else{
				$content_table='</tr>';
			...
			$table='<div style="opacity: 1; transform: none;">
				<div class="flex justify-between items-center mb-8">
					...
					<button
						id="modal_user"
						onClick="javascript:modal_insert_user()"
