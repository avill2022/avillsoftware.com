<?php
    ...
    $query="UPDATE users SET username='$name',email='$email',type=$type WHERE id=$id";
	//$response['status'] = $name.' '.$email.' '.$password.' '.$type;
    if($password != ''){
        $pass_hash = password_hash($password, PASSWORD_BCRYPT);
        $query="UPDATE users SET username='$name',email='$email',password='$pass_hash',type=$type WHERE id=$id";
    }
