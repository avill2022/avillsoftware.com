<?php
	...
	if (!isset($_SESSION["event_id"]) && !isset($_SESSION["email"]))
									...
									<div>
										...
										<div
											...
											<div data-side="bottom" data-align="center" role="menu" aria-orientation="vertical" data-state="open"
												...
												<div 
													id="edit_event'.$id.'"
													onclick="javascript:modal_edit_event('. $id .','.'\'' . addslashes($eventname) .'\','.'\'' . addslashes($email) .'\','. $type .',\'' . addslashes($created_at) .'\')"
													role="menuitem"
												...
												<div id="delete_event'.$id.'"
													onclick="javascript:delete_event('.$id.')"
													role="menuitem"
	...
	try{
		$result=mysqli_query($dbh, "select * from events;");
		...
		if ($result){
			if ($total>0){
				...
				while ($ren=mysqli_fetch_array($result)){
					$content_table.= get_item($ren['id'],$ren['eventname'],$ren['email'],$ren['created_at'],$ren['type']);
				}
			}
			else{
				$content_table='</tr>';
			...
			$table='<div style="opacity: 1; transform: none;">
				<div class="flex justify-between items-center mb-8">
					...
					<button
						id="modal_event"
						onClick="javascript:modal_insert_event()"
